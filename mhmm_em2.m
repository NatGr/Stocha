...
while (num_iter <= max_iter) & ~converged
  ...
  if final_initial_shared
    % force last state to emit the same as the initial
    postmix(1,:) = postmix(1,:) + postmix(end,:);
    postmix(end,:) = postmix(1,:);
    m(:,1,:) = m(:,1,:) + m(:,end,:);
    m(:,end,:) = m(:,1,:);
    ip(1,:) = ip(1,:) + ip(end,:);
    ip(end,:) = ip(1,:);
...
for ex=1:numex
  ...
  if mix
	...
	fwdback2(prior, posterior, transmat, B, 'obslik2', B2, 'mixmat', mixmat);
  else
    B = mixgauss_prob(obs, mu, Sigma);
    [alpha, beta, gamma,  current_loglik, xi_summed] = fwdback2(prior, posterior, transmat, B);
  end    
