...
function path = mhmm_viterbi(obs, model)
% obs 		observation				(numberCep x T)
	...
	if model.pi(i) ~= 0 	% shortcut for 0
		...
		for k = 1:numberGaussPerState
			if model.B(i,k) ~= 0 	% shortcut for 0
				acc = acc + model.B(i,k) * gaussian_prob(obs(:,1), model.mu(:,i,k), model.sigma(:,:,i,k));
			end
		end
		...
		if currentMaxValue ~= 0 	% shortcut for 0
			...
			for k = 1:numberGaussPerState
				if model.B(j,k) ~= 0 	% shortcut for 0
					acc = acc + model.B(j,k) * gaussian_prob(obs(:,t), model.mu(:,j,k), model.sigma(:,:,j,k));
				end
			end
